...
**Repo is going private for a few weeks (2026-07-23), planned to go back public after.** Read this before doing anything else if you're picking the project back up during this window:

- The live dashboard (`https://pdglenchur-glitch.github.io/market_ai_pulse/`) will 404 while private — GitHub Pages on the free plan requires a public repo. This is expected, not a bug to chase.
- **The pipeline itself should keep working.** The transform step (`git_source`) needs to authenticate to clone a private repo, so a Databricks Git credential was registered beforehand (credential ID `424336786409579`, GitHub user `pdglenchur-glitch`, backed by a fine-grained PAT scoped read-only to just this repo). Ingestion and publish were never affected by repo visibility in the first place (R2/Databricks writes and `git push` to your own private repo both work regardless). So historical data should keep accumulating in the private repo the whole time — the dashboard should come back already caught-up the moment it goes public again, not restart from scratch.
- If the PAT expires before going public again, the transform step will start failing (`git_source` clone auth error) and a `pipeline-failure` issue will open. Fix: generate a new fine-grained PAT the same way, add it as the `DATABRICKS_GIT_PAT` GitHub secret, then re-run `client.git_credentials.create(git_provider="gitHub", personal_access_token=..., git_username="pdglenchur-glitch")` via the Databricks SDK (delete the old credential first, same idempotent pattern as the one-off registration script used originally — see the commit that added and then removed `orchestration/_register_git_credential.py`).
- **When going back to public**: should self-heal automatically (Pages resumes, `git_source` still works fine on a public repo even with the credential still registered — it's just unnecessary at that point, not harmful). Recommend one manual pipeline dispatch afterward to confirm, same as always — don't just assume.
