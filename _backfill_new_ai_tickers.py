"""One-off historical backfill for the six tickers added to the AI basket
on 2026-07-29 (TSM, AMAT, MU, CRWV, BE, ANET) - lands their market history
directly into the silver layer so they have the same depth as the rest of
the basket and every window (7D/30D/90D/All) works for them immediately.

Same pattern as the 2026-07-28 backfill (deleted after its one-time use,
same pattern recovered from git history commit 315d008 the first time):
writes straight to workspace.silver.market_data via the SQL warehouse
connector, bypassing bronze/R2, keyed by MERGE on (symbol, date) so it's
safe to re-run. Only touches these six new symbols.

CoreWeave (CRWV) IPO'd 2025-03-28, before this backfill's start date, so
it has full history across the whole window like every other symbol here -
no partial-history gap to handle.
...
NEW_SYMBOLS = ["TSM", "AMAT", "MU", "CRWV", "BE", "ANET"]
...
def backfill_market_data(cursor) -> None:
    ...
    stage = "workspace.silver._backfill_stage_new_ai_tickers_2"
    cursor.execute(
        f"""
