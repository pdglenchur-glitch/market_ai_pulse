"""One-off historical backfill for the two tickers added to the AI basket
on 2026-07-31 (CEG, SMCI) - lands their market history directly into the
silver layer so they have the same depth as the rest of the basket and
every window (7D/30D/90D/All) works for them immediately.

Same pattern as the two prior basket-expansion backfills (each deleted
after its one-time use, this script rebuilt from that pattern each time,
originally recovered from git history commit 315d008): writes straight to
workspace.silver.market_data via the SQL warehouse connector, bypassing
bronze/R2, keyed by MERGE on (symbol, date) so it's safe to re-run. Only
touches these two new symbols.

Both CEG and SMCI have full trading history well before this backfill's
start date, confirmed via yfinance before writing this script - no
partial-history gap to handle.
...
def backfill_market_data(cursor) -> None:
    ...
    stage = "workspace.silver._backfill_stage_new_ai_tickers_3"
    cursor.execute(
        f"""
