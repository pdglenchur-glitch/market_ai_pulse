...
The charts above are backed by a full year of history, backfilled once from each source's own historical API and accumulating since. The pipeline runs daily (see [Design decisions](#design-decisions)); each run re-captures a trailing window of market data, so a run that GitHub's scheduler fires late or skips self-heals on the next one and the price panels stay gap-free.
...
<!-- KEY_FINDINGS_START -->
- The AI basket has outperformed the S&P 500 by **+59.4 points** over the trailing year (+81.1% vs. +21.8%). The spread is up 4.2pts versus last week's 55.2pts (2026-08-24).
- Routine day-to-day AI-sector search attention shows no reliable link to stock moves (strongest lag correlation only +0.17), but the biggest attention spikes show a **+6.0-point** swing between the return before and after each one. That swing is flat versus last week's 6.0pts (2026-08-24).
- **NVDA** still accounts for **30%** of the AI basket's trading volume, though the basket-wide Herfindahl index of 1286 is now unconcentrated overall. NVDA's share is up 0.2% versus last week's 29.3% (2026-08-24).
- **10 of 11** sectors fell on rising-yield days and rallied on falling-yield days as expected, with XLE the exception. The count following the pattern is flat versus last week's 10 sectors (2026-08-24).
- Realized volatility peaked at **1.3%** on 2026-04-09 and has since eased to **0.60%** as of the latest reading. The latest reading is down 0.24% versus last week's 0.84% (2026-08-24).
- The infrastructure names added to the AI basket returned **+119.6%** on an equal-weighted basis versus **+41.3%** for the original core cohort, though 3 of those infrastructure names are actually down over the same window. The gap is up 1.8pts versus last week's 76.5pts (2026-08-24).
<!-- KEY_FINDINGS_END -->
...
One more notebook exists alongside this chain: [`analysis/key_findings.ipynb`](analysis/key_findings.ipynb) is a separate investment analysis. It runs as an additive Monday-gated step *after* the dashboard publish, gated to only ever touch itself and never the dashboard-facing steps above. The analysis stays weekly even though the pipeline runs daily: the Monday gate fires it on one pipeline run a week and skips it the other six days (and on any mid-week `workflow_dispatch`). It's also safe to run by hand anytime. The `query_*.py` files in [`databricks/`](databricks/) and the connectivity checks in [`scripts/`](scripts/) are one-off diagnostics from early development, not part of the automated pipeline either.
...
**Daily cadence.** The pipeline ran daily from 2026-07-22 to 2026-09-01, reverted to a weekly Monday cron after Databricks Free Edition started refusing job runs ("Triggering new runs ... is currently disabled temporarily"), then went back to daily on 2026-09-07. The weekly revert was aimed at a suspected serverless compute throttle, but the block turned out to be specific to the Jobs API `run-now` call: manual runs from the Databricks UI and the SQL warehouse worked fine throughout, and a single weekly run still hit the error. Switching the transform to a file-arrival trigger (next entry) removed the `run-now` call entirely, which is what made daily viable again. The residual unknown is whether Free Edition throttles serverless compute minutes at roughly 30 job runs a month; if that resurfaces, the rollback is a one-line cron change back to `0 13 * * 1`. `pull_market_data.py` still re-captures a trailing window (about 10 trading days) and `bronze_to_silver.py` merges by `(symbol, date)`, so the price panels stay gap-free even when GitHub's scheduler fires the cron late or skips it.

**The dashboard is static HTML/JS reading a JSON file.** No dashboard-side database credentials to secure, nothing to keep warm, and it hosts for free on GitHub Pages. The tradeoff is that the latest values are only as fresh as the last daily run, which is the right call for a portfolio piece: the point is the end-to-end pipeline, not real-time data, and the retroactive trailing-window fetches mean the *history* the charts show stays complete regardless of when you look.
